    ...
    $.widget("ui.triggeredAutocomplete", $.extend(true, {}, $.ui.autocomplete.prototype, {
        ...
        _create:function() {
            ...
            this.options.select = function(event, ui) {
                ...
                this.value = start + self.options.trigger+ui.item.name+' ' + end;
                self.element.scrollTop(top);

                // Create an id map so we can create a hidden version of this string with id's instead of names.

                self.id_map[ui.item.name] = ui.item.value;
                self.updateHidden();

                /** Places the caret right after the inserted item. */
                var index = start.length + self.options.trigger.length + ui.item.name.length + 2;
        ...
        // If there is an 'img' then show it beside the name.

        _renderItem:  function( ul, item ) {
          return $( "<li></li>" )
              .data( "item.autocomplete", item )
              .append( $("<img></img>").attr( {'src': item.image } ) )
              .append( $( "<a></a>" ).text( item.name ).append() )
              .appendTo( ul );
        },
                        ...
                        success: function(data) {
                            if(data != null) {
                                response($.map(data, function(item) {
                                    ...
                                    if(!self.id_map[name] || self.options.allowDuplicates) {
                                        return item
                                    }
                                }));
                                self.stopLength = -1;
